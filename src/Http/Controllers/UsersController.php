...
class UsersController extends Controller
{
    ...
    {
        $query = User::leftJoin('permission_packages', 'users.permission_package_id', 'permission_packages.id')->select('users.*', 'permission_packages.name as package_name')->where('admin', 0)->where('users.id', '!=', Auth::user()->id);
